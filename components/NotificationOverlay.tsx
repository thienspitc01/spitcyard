...
const NotificationOverlay: React.FC = () => {
  ...
  useEffect(() => {
    // Yêu cầu quyền thông báo hệ thống khi khởi tạo
    ...
    const handleNotify = (e: any) => {
      const { title: msgTitle, body: msgBody } = e.detail;
      setTitle(msgTitle);
      setMessage(msgBody);
      setIsVisible(true);

      // Đẩy thông báo ra ngoài hệ điều hành (System Notification)
      if ("Notification" in window && Notification.permission === "granted") {
        new Notification(msgTitle, {
          body: msgBody,
          icon: "/vite.svg", // Có thể thay bằng icon port
          tag: "port-urgent-alert",
          requireInteraction: true // Thông báo sẽ không tự biến mất cho đến khi tương tác
        });
      }
    };

    window.addEventListener('port-notification', handleNotify);
    return () => window.removeEventListener('port-notification', handleNotify);
  }, []);
    ...
    <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-red-600/95 backdrop-blur-2xl animate-in fade-in duration-300">
      <div className="text-center p-12 max-w-3xl w-full mx-4">
        ...
        <p className="mt-8 text-white/60 font-bold uppercase tracking-widest text-sm italic">
          Bấm nút trên để tắt báo động âm thanh và đóng cảnh báo
        </p>
      </div>
    </div>
  );
