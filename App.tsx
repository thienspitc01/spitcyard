...
const App: React.FC = () => {
  ...
  const handleScheduleChange = async (newSchedule: ScheduleData[]) => {
      ...
      if (isCloudConnected) {
          await syncTable('yard_schedule', 'schedule_snapshot', {
  ...
  const handleClearData = async () => {
    if (window.confirm('Xóa toàn bộ dữ liệu hiện tại? Thao tác này sẽ đồng bộ xóa cho mọi người dùng khác.')) {
        setContainers([]);
        setStats(null);
        setVessels([]);
          ...
          {mode === 'YARD' && (
            <YardDashboard 
              ...
              onAssign={(id, loc) => {
                const req = requests.find(r => r.id === id);
                if (req) {
                  const updatedReq = { ...req, status: 'assigned' as const, assignedLocation: loc, acknowledgedByGate: false };
                  if (isCloudConnected) syncTable('yard_requests', id, updatedReq);
                }
              }} 
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-center flex-col items-center gap-2">
                        ...
                        {stats && (
                            <div className="flex items-center space-x-4 text-[10px] text-slate-500 font-bold uppercase">
                                ...
                                <button onClick={handleClearData} className="text-red-500 hover:underline">Clear Data</button>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="bg-white p-4 rounded-xl shadow border border-slate-200 space-y-4">
                      <div className="flex flex-col md:flex-row gap-4 items-end">
                        <div className="flex-1 space-y-1 w-full">
                          ...
                          <input 
                            ...
                            className="w-full px-4 py-2 border rounded-md text-sm outline-none focus:ring-1 ring-blue-500" 
                            placeholder="Enter container number or location" 
                          />
                        </div>
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] font-bold text-slate-500 uppercase">Vessel Highlight Filters</label>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          {[0, 1, 2].map(idx => (
                            <div key={idx} className="flex items-center gap-2">
                              <div className={`w-3 h-3 rounded-full flex-shrink-0 ${idx === 0 ? 'bg-sky-500' : idx === 1 ? 'bg-lime-500' : 'bg-amber-500'}`}></div>
                              <select 
                                value={selectedVessels[idx]} 
                                onChange={(e) => {
                                  const newV = [...selectedVessels];
                                  newV[idx] = e.target.value;
                                  setSelectedVessels(newV);
                                }} 
